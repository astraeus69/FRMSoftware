function descargarPDF_RegistroJarras() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Movimiento de registro de jarras", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("movimiento_registro_jarras.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_RegistroJarras() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "RegistroJarras");
    XLSX.writeFile(wb, "movimiento_registro_jarras.xlsx");
}
