function descargarPDF_GestionViajes() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Movimiento de gestion de viajes", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("movimiento_gestion_viajes.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_GestionViajes() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "GestionViajes");
    XLSX.writeFile(wb, "movimiento_gestion_viajes.xlsx");
}
