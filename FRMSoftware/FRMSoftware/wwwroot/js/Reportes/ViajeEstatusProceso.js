function generarGraficaViajesProcesoEstatus(labels, values) {
    const ctx = document.getElementById('chartViajesPorEstatusProceso').getContext('2d');
...
function descargarPDF_ViajesProcesoEstatus() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Reporte de viajes de proceso por estatus", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        const canvas = document.getElementById("chartViajesPorEstatusProceso");
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text("Gráfica viajes de proceso por estatus", pageWidth / 2, finalY, { align: "center" });
        finalY += 5;

        doc.addImage(imgData, "PNG", marginX, finalY, imgWidth, imgHeight);

        doc.save("reporte_viajes_proceso_por_estatus.pdf");
    } else {
...
function descargarExcel_ViajesCajasEstatus() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Rep. Viajes Proc. Est.");
    XLSX.writeFile(wb, "reporte_viajes_proceso_por_estatus.xlsx");
}
