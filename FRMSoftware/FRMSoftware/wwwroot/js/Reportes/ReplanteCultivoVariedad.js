window.generarGraficaReplantesPorCultivoVariedad = function (idCanvas, labels, values, colorIndex) {
    ...
    window[`grafica_${idCanvas}`] = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: context => `${context.parsed.y} replantes`
                    }
                },
                datalabels: {
                    display: true,
...
function descargarPDF_ReplantesPorCultivoVariedad() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Reporte de replantes por cultivo y variedad", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("reporte_replantes_por_cultivo.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_ReplantesPorCultivoVariedad() {
    var ws = XLSX.utils.table_to_sheet(document.querySelector("table"));
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rep. Cult. Var.");
    XLSX.writeFile(wb, "reporte_replantes_por_cultivo.xlsx");
}
