function generarGraficaProcesoLlaves(labels, values) {
    const ctx = document.getElementById('chartKilosPorLlave').getContext('2d');
                ...
                tooltip: {
                    callbacks: {
                        label: function (context) {
                            return `${context.parsed.x} kilos de proceso`;
                        }
                    }
                },
...
function descargarPDF_ProcesoLlaves() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Reporte de kilos de proceso por llave", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.autoTable({
            ...
            didParseCell: function (data) {
                if (data.section === 'body') {
                    ...
                    if (texto.includes('Total general de kilos procesados:')) {
                        // Fila total general
        ...
        // Gráfico
        const canvas = document.getElementById("chartKilosPorLlave");
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text("Gráfica de kilos de proceso por llave", pageWidth / 2, finalY, { align: "center" });
        finalY += 5; // Espacio entre el título y la imagen

        doc.addImage(imgData, "PNG", marginX, finalY, imgWidth, imgHeight);

        // Guardar PDF
        doc.save("reporte_proceso_por_llave.pdf");
    } else {
...
function descargarExcel_ProcesoLlaves() {
    // Crear una hoja de Excel
    var ws = XLSX.utils.table_to_sheet(document.querySelector("table"));
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rep. Proceso Llave");

    // Descargar el archivo Excel
    XLSX.writeFile(wb, "proceso_por_llave.xlsx");
}
