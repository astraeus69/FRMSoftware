function descargarPDF_Vehiculos() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Catálogo de vehículos", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("catalogo_vehiculos.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_Vehiculos() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Vehículos");
    XLSX.writeFile(wb, "catalogo_vehiculos.xlsx");
}
