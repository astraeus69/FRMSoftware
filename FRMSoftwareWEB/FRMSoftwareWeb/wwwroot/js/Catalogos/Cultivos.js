function descargarPDF_Cultivos() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Catálogo de cultivos", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("catalogo_cultivos_variedades.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_Cultivos() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Cultivos");
    XLSX.writeFile(wb, "catalogo_cultivos.xlsx");
}
