function descargarPDF_Viveros() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Catálogo de viveros", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("catalogo_viveros.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_Viveros() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Viveros");
    XLSX.writeFile(wb, "catalogo_viveros.xlsx");
}
