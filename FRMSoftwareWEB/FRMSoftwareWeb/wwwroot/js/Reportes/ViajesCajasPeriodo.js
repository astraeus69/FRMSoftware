function generarGraficaViajesCajasPeriodo(labels, values) {
    const ctx = document.getElementById('chartCajasPorPeriodo').getContext('2d');
...
let aceptadosGlobalesCajas = 0;
let rechazadosGlobalesCajas = 0;

function generarGraficaEstadoViajesCajas(aceptados, rechazados) {
    // Actualizar las variables globales con los valores actuales
    aceptadosGlobalesCajas = aceptados;
    rechazadosGlobalesCajas = rechazados;

    const canvas = document.getElementById("chartEstadoViajesCajas");
    ...
    const resumenElement = document.getElementById("resumenEstadoViajesCajas");
...
function descargarPDF_ViajesCajasPorPeriodo() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Reporte de viajes de cajas por periodo", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.autoTable({
            ...
            didParseCell: function (data) {
                if (data.section === 'body') {
                    ...
                    if (texto.includes('Total general de cajas del periodo:')) {
                        // Fila total general
        ...
        const canvasCajas = document.getElementById("chartCajasPorPeriodo");
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text("Gráfica de cajas por periodo", pageWidth / 2, 20, { align: "center" });
        ...
        const total = aceptadosGlobalesCajas + rechazadosGlobalesCajas;
        const resumen = `Aceptados: ${aceptadosGlobalesCajas} viajes   |   Rechazados: ${rechazadosGlobalesCajas} viajes   |   Total: ${total} viajes`;
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text("Gráfica de viajes de cajas aceptados vs rechazados", pageWidth / 2, 20, { align: "center" });
        ...
        const canvasViajes = document.getElementById("chartEstadoViajesCajas");
        ...
        // Guardar PDF
        doc.save("reporte_viajes_cajas_por_periodo.pdf");
    } else {
...
function descargarExcel_ViajesCajasPorPeriodo() {
    // Crear una hoja de Excel
    var ws = XLSX.utils.table_to_sheet(document.querySelector("table"));
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rep. Viajes Cajas Per.");

    // Descargar el archivo Excel
    XLSX.writeFile(wb, "viajes_cajas_por_periodo.xlsx");
}
