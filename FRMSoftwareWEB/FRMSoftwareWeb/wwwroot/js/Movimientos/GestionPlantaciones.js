function descargarPDF_GestionPlantaciones() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Movimiento de gestión de plantaciones", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("movimiento_gestion_plantaciones.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_GestionPlantaciones() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "GestionPlantaciones");
    XLSX.writeFile(wb, "movimiento_gestion_plantaciones.xlsx");
}
