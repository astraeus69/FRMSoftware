window.generarGraficaProcesoPorCultivoVariedad = function (idCanvas, labels, values, colorIndex) {
...
function descargarPDF_ProcesoPorCultivoVariedad() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Reporte de proceso por cultivo y variedad", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("reporte_proceso_por_cultivo.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_ProcesoPorCultivoVariedad() {
    var ws = XLSX.utils.table_to_sheet(document.querySelector("table"));
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Proc. Cult. Var.");
