function descargarPDF_Ranchos() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Catálogo de ranchos", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("catalogo_ranchos.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_Ranchos() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Ranchos");
    XLSX.writeFile(wb, "catalogo_ranchos.xlsx");
}
