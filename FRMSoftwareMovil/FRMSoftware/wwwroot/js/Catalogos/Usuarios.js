function descargarPDF_Usuarios() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Catálogo de usuarios", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("catalogo_usuarios.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_Usuarios() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Usuarios");
    XLSX.writeFile(wb, "catalogo_usuarios.xlsx");
}
