function descargarPDF_RegistroVentas() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Movimiento de registro de ventas", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("movimiento_registro_ventas.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_RegistroVentas() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "RegistroVentas");
    XLSX.writeFile(wb, "movimiento_registro_ventas.xlsx");
}
