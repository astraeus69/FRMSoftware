function descargarPDF_ClasificarProceso() {
    ...
    if (doc.autoTable) {
        ...
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Movimiento de clasificación de proceso", doc.internal.pageSize.width / 2, 35, { align: "center" });
        ...
        doc.save("movimiento_clasificar_proceso.pdf");
    } else {
        alert("jsPDF AutoTable no está disponible");
    }
}

function descargarExcel_ClasificarProceso() {
    const tabla = document.querySelector(".table");
    const ws = XLSX.utils.table_to_sheet(tabla);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "ClasificarProceso");
    XLSX.writeFile(wb, "movimiento_clasificar_proceso.xlsx");
}
